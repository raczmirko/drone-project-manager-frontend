// src/features/operations/components/OperationImageMetadataDashboard.tsx
...
export default function OperationImageMetadataDashboard({
    ...
    if (!data) {
        return (
            <Alert severity="info">
                {t('operations.imageAnalysis.dashboard.noData')}
            </Alert>
        );
    }
    ...
    return (
        ...
        >
            <Paper variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
                <Stack spacing={2}>
                    <Typography variant="h6">
                        {t('operations.imageAnalysis.dashboard.altitudeProfile')}
                    </Typography>
                    ...
                    <LineChart
                        ...
                        xAxis={[
                            {
                                data: data.altitudeProfile.map((item) => item.sequence),
                                label: t('operations.imageAnalysis.dashboard.captureOrder'),
                            },
                        ]}
                        series={[
                            {
                                data: data.altitudeProfile.map((item) => item.altitude),
            ...
            <Paper variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
                <Stack spacing={2}>
                    <Typography variant="h6">
                        {t('operations.imageAnalysis.dashboard.altitudeDistribution')}
                    </Typography>
                    ...
                    <BarChart
                        ...
                        xAxis={[
                            {
                                scaleType: 'band',
                                data: data.altitudeDistribution.map((item) => item.bucketLabel),
                            },
                        ]}
                        series={[
                            {
                                data: data.altitudeDistribution.map((item) => item.count),
            ...
            <Paper variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
                <Stack spacing={2}>
                    <Typography variant="h6">
                        {t('operations.imageAnalysis.dashboard.distanceAltitude')}
                    </Typography>
                    ...
                    <LineChart
                        ...
                        xAxis={[
                            {
                                data: data.distanceAltitudeProfile.map((item) => item.distanceMeters),
                                label: t('operations.imageAnalysis.dashboard.distanceMeters'),
                            },
                        ]}
                        series={[
                            {
                                data: data.distanceAltitudeProfile.map((item) => item.altitude),
                                label: t('operations.imageAnalysis.dashboard.altitudeMeters'),
            ...
            <Paper variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
                <Stack spacing={2}>
                    <Typography variant="h6">
                        {t('operations.imageAnalysis.dashboard.groundTrack')}
                    </Typography>
                        ...
                        series={[
                            {
                                ...
                                data: data.groundTrack.map((item) => ({
                                    id: item.sequence,
                                    x: item.longitude,
                                    y: item.latitude,
                                })),
